...
class Login extends CI_Controller
{
	...
	function _validate_login($str)
	{
		...
		if ($query->num_rows() > 0) {
			...
			if ($this->input->post('login_type') == 'customercare') {
				$this->session->set_userdata('login_type', 'customercare');
				$this->session->set_userdata('customercare_login', '1');
				$this->session->set_userdata('customercare_id', $row->customercare_id);
			}
			return TRUE;
		} else {
                    $memessage = 'failed';
