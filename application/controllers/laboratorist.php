...
class laboratorist extends CI_Controller
{
	    ...
	    function manage_email_sent($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('laboratorist_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
                if($param1 == 'delete_email' && $param2 == 'sent')
                {
                  ...
                  redirect(base_url() . 'index.php?laboratorist/manage_email_sent', 'refresh');
                }
                
               
           
                $this->db->where(array('user_from_type'=>'6', 'user_from_id'=>$this->session->userdata('laboratorist_id')));
		...
		$page_data['page_name']  = 'manage_email_sent';
                $page_data['laboratorist_id'] = $this->session->userdata('laboratorist_id');
		...
		if($param1 == 'delete_email' && $param2 == 'inbox')
                {
                  $this->db->where('message_id',$param3);
                  $this->db->delete('message');
                  redirect(base_url() . 'index.php?laboratorist/manage_email', 'refresh');
                }
                
                
                $this->db->where(array('user_to_type'=>'6', 'user_to_id'=>$this->session->userdata('laboratorist_id')));
                $this->db->order_by('message_id' , 'desc'); 
